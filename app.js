...
function onInvoiceQRDecoded(raw){
  ...
  body.innerHTML=`
    <div style="background:linear-gradient(135deg,rgba(0,188,212,0.12),rgba(91,72,232,0.08));border:1.5px solid rgba(0,188,212,0.3);border-radius:var(--r-sm);padding:14px;margin-bottom:12px">
...
function renderYearSummary(){
  ...
  el.innerHTML=`
    <!-- 年度總覽 -->
    <div style="background:${totalSurplus>=0?'linear-gradient(135deg,rgba(24,184,124,0.1),rgba(91,72,232,0.07))':'linear-gradient(135deg,rgba(232,48,48,0.08),rgba(232,124,32,0.06))'};border:2px solid ${totalSurplus>=0?'rgba(24,184,124,0.3)':'rgba(232,48,48,0.3)'};border-radius:var(--r);padding:16px;margin-bottom:10px;text-align:center">
...
function renderSankeyCard(year, totalIncome, topCats, totalCashSp, totalSurplus){
  ...
  // 多色色盤（避免全部一色）
  const palette=['#e8488a','#5b48e8','#18b87c','#e87c20','#3b82f6','#a855f7','#14b8a6'];
...
function renderNetWorthTimelineCard(){
  ...
  return `
    <div class="chart-card" style="margin-top:12px">
      ...
      <svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto;display:block">
        <path d="${area}" fill="rgba(91,72,232,0.12)"/>
        <path d="${path}" fill="none" stroke="#5b48e8" stroke-width="2"/>
        ${points.map((p,i)=>`<circle cx="${xs(i)}" cy="${ys(p.val)}" r="2.5" fill="#5b48e8"/>`).join('')}
        ${xlabels}
      </svg>
...
function exportYearReviewCard(year){
  try{
    ...
    g.addColorStop(0,'#5b48e8'); g.addColorStop(1,'#e8488a');
...
function renderSummary(){
  ...
  summContentEl.innerHTML=`
    ...
    <!-- surplus block -->
    <div style="background:${surplus>=0?'linear-gradient(135deg,rgba(24,184,124,0.1),rgba(91,72,232,0.07))':'linear-gradient(135deg,rgba(232,48,48,0.08),rgba(232,124,32,0.06))'};border:2px solid ${surplus>=0?'rgba(24,184,124,0.3)':'rgba(232,48,48,0.3)'};border-radius:var(--r);padding:16px;margin-bottom:14px;text-align:center">
...
function renderYearlySavings(){
  ...
  el.innerHTML=`
    <div style="background:linear-gradient(135deg,rgba(24,184,124,0.1),rgba(91,72,232,0.07));border:1.5px solid rgba(24,184,124,0.25);border-radius:var(--r);padding:14px;margin-bottom:12px;text-align:center">
...
function renderBonusTab(){
  ...
  bc.innerHTML=!sorted.length
    ...
    :sorted.map(b=>{
      ...
      const modeBadge={budget:'<span class="record-tag" style="background:#ddd6fe;color:#5b48e8">📊 當月可用</span>',travel:'<span class="record-tag" style="background:#dbeafe;color:#2563eb">✈️ 旅遊額度</span>',savings:'<span class="record-tag" style="background:#fde9f2;color:#e8488a">🐷 存款</span>'}[mode]||'';
      return `<div style="display:flex;align-items:center;gap:10px;padding:9px 12px;background:${isCur?'var(--accent3-light)':'var(--surface)'};border:1.5px solid ${isCur?'rgba(24,184,124,0.3)':'var(--border)'};border-radius:var(--r-sm);margin-bottom:7px;box-shadow:var(--shadow)">
...
function renderInvModal(pid){
  ...
  const calibrateBanner=(avgDays>0&&Math.abs(avgDays-p.totalDays)/p.totalDays>=0.25)?`
    <div style="background:linear-gradient(135deg,rgba(91,72,232,0.08),rgba(91,72,232,0.02));border:1px solid rgba(91,72,232,0.25);border-radius:var(--r-sm);padding:10px 12px;margin-bottom:12px;display:flex;gap:10px;align-items:center">
      <div style="font-size:18px">📊</div>
...
function openCompareModal(pid){
  ...
  document.getElementById('cmpContent').innerHTML=`
    <div style="background:var(--accent-light);border:1px solid rgba(91,72,232,0.2);border-radius:var(--r-sm);padding:10px 12px;margin-bottom:14px;font-size:12px;color:var(--accent);font-weight:500;line-height:1.5">
...
function renderOnboardStep(){
  ...
  if(step.key==='ai'){
    html+=`<div style="background:linear-gradient(135deg,#5b48e8,#8b6bf2);color:#fff;border-radius:12px;padding:12px 14px;font-size:11px;line-height:1.7;margin-bottom:10px">
...
function _buildXlsHtml(sheetName, headers, rows){
  const headHtml=headers.map(h=>`<th style="background:#5b48e8;color:#fff;padding:6px 10px;text-align:left">${escapeHTML(h)}</th>`).join('');
...
function _printHtml(title, bodyHtml){
  ...
  w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${escapeHTML(title)}</title>
    <style>
      ...
      h1{font-size:22px;margin:0 0 4px;color:#5b48e8}
      h2{font-size:14px;margin:18px 0 8px;border-left:4px solid #5b48e8;padding-left:8px}
      ...
      th{background:#f0eef9;color:#5b48e8;text-align:left;padding:6px 8px;border:1px solid #ddd;font-weight:700}
      ...
      .kpi .v{font-family:'DM Mono',monospace;font-size:16px;font-weight:700;color:#5b48e8;margin-top:3px}
    ...
    <button class="no-print" onclick="window.print()" style="position:fixed;top:10px;right:10px;padding:8px 14px;background:#5b48e8;color:#fff;border:0;border-radius:6px;cursor:pointer">🖨️ 列印 / 存為 PDF</button>
    ${bodyHtml}
